import Leaflet, { DivIcon, Point } from 'leaflet'
import { Dimensions, MapMarker, MapMarkerAnimation } from './model'
...
const getIconFromEmojiOrImageOrSVG = (icon: string, size: Dimensions) => {
  let [x, y] = size
  if (icon.includes('svg') || icon.includes('SVG')) {
    return `<div style='font-size: ${Math.max(x, y)}px'>${icon}</div>`
  } else if (icon.includes('//') && icon.includes('http')) {
    return `<img src="${icon}" style="width:${x}px;height:${y}px;">`
  } else if (icon.includes('base64')) {
    return `<img src="${base64Image}" style="width:${x}px;height:${y}px;">`
  } else {
    return `<div style='font-size: ${Math.max(x, y)}px'>${icon}</div>`
  }
}
